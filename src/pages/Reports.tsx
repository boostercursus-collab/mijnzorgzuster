...
const Reports: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setAssignments(assignmentsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as any))); // Gebruik any voor flexibele veldnamen
  ...
  // BEREKENING: (Uren * rate) * 0.10
  const totalCommission = filteredRegistrations.reduce((acc, reg) => {
    const assignment = assignments.find(a => a.id === reg.assignmentId);
    const rateValue = Number(assignment?.rate) || 0; // GEWIJZIGD NAAR 'rate'
  ...
  const generatePDF = async () => {
    ...
    } catch (e) { console.warn("Logo niet geladen in PDF"); }
    ...
    <div className="p-8 space-y-8 max-w-7xl mx-auto">
      <header className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 font-medium">Berekening van de 10% fee per opdracht.</p>
        </div>
        <button onClick={generatePDF} className="bg-black text-white px-8 py-4 rounded-2xl flex gap-2 text-xs font-black uppercase shadow-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-pink-600 p-8 rounded-[2.5rem] text-white shadow-xl flex items-center justify
