...
const Assignments: React.FC = () => {
  ...
  const [newAssignment, setNewAssignment] = useState({
    title: '',
    clientId: '',
    uid: '', // Dit is de geselecteerde ZZP'er
    startDate: '',
    endDate: '',
    rate: ''
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await addDoc(collection(db, 'assignments'), {
        ...newAssignment,
        rate: parseFloat(newAssignment.rate),
      ...
      alert("Opslaan mislukt. Controleer de console.");
    } finally {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-8">
      <div className="flex items-center gap-4">
        <div className="p-3 bg-pink-600 rounded-2xl text-white">
          <Briefcase size={32} />
        </div>
        <div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-gray-100 space-y-8">
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <input 
              ...
              className="w-full p-5 bg-gray-50 rounded-2xl font-bold border-none outline-none focus:ring-2 ring-pink-500/20"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Klant Selectie */}
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest ml-2">Klant / Instelling</label>
              <select 
                required
                className="w-full p-5 bg-gray-50 rounded-2xl font-bold border-none outline-none focus:ring-2 ring-pink-500/20 appearance-none"
              ...
              >
                ...
                {clients.map(c => (
                  <option key={c.id} value={c.id}>{c.name || c.companyName || 'Naamloze klant'}</option>
                ))}
              </select>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest ml-2">Tarief per uur</label>
              <div className="relative">
                <Euro className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
                <input 
                  type="number" 
                  placeholder="0.00"
                  className="w-full p-5 pl-12 bg-gray-50 rounded-2xl font-bold border-none outline-none"
                  value={newAssignment.rate}
                  onChange={(e) => setNewAssignment({...newAssignment, rate: e.target.value})}
                />
              </div>
            </div>
          </div>
        ...
        <button 
          ...
          className="w-full bg-[#111827] text-white py-6 rounded-3xl font-black flex items-center justify-center gap-3 hover:bg-black transition-all shadow-xl uppercase tracking-widest disabled:opacity-50"
        >
